...
uses
  ...
  GLScene, GLOBjects, GLHUDOBjects, GLMaterial, GLWindowsFont, GLTexture,
  VectorGeometry, VectorTypes, GLCrossPlatform;
...
begin
  ...
  origin[0] := dfGLSceneObjects.Viewer.Width + C_INDICATOR_OFFSET_X;
  origin[1] := dfGLSceneObjects.Viewer.Height + C_INDICATOR_OFFSET_Y;

  for i := 0 to FCellCount - 1 do
  begin
    FCells[i] := TGLHUDSprite.CreateAsChild(Self);
    cellPos[0] := - C_CELL_RADIUS * cos((pi / 180) * stepAngle * i);
    cellPos[1] := - C_CELL_RADIUS * sin((pi / 180) * stepAngle * i);
    cellPos[2] := 0.5;
    FCells[i].Rotation := (180 / pi) * ArcCos(VectorAngleCosine(YVector, cellPos));
    if cellPos[0] < origin[0] then
      FCells[i].Rotation := - FCells[i].Rotation;

    cellPos[0] := cellPos[0] + origin[0];
    cellPos[1] := cellPos[1] + origin[1];
  ...
  FSpeedText.Position.SetPoint(origin[0], origin[1], 0.5);
  FSpeedText.ModulateColor.SetColor(0, 1, 0, 0.8);
end;

function TdfSpeedIndicator.GetActiveColor: TVector;
begin
  Result[0] := C_CELL_ACTIVE_R;
  Result[1] := C_CELL_ACTIVE_G;
  Result[2] := C_CELL_ACTIVE_B;
  Result[3] := C_CELL_ACTIVE_A;
end;
...
begin
  Result[0] := C_CELL_INACTIVE_R;
  Result[1] := C_CELL_INACTIVE_G;
  Result[2] := C_CELL_INACTIVE_B;
  Result[3] := C_CELL_INACTIVE_A;
end;
