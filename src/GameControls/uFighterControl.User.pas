...
uses
  Windows, SysUtils, IniFiles, Contnrs,

  uSpacefighter, uLog, dfKeyboardExt, uFighterControl, uSpeedIndicator, uFonts,
  uTarget, uGameObject,

  GLScene, GLKeyboard, VectorGeometry, VectorTypes, GLHUDObjects, GLObjects,
  GLMaterial, GLTexture, GLFilePNG;
...
const
  ...
  C_RETURN_ROTATE_SPEED = 1;
  C_RETURN_ROTATE_MINDISTANCE = 0.001;
  ...
  C_ACTION_ASYNC: array[0..C_ACTION_COUNT - 1] of Boolean =
...
begin
  ...
  begin
    ...
    AddFontToTexts(GetFont(C_FONT_1));
  end;
...
procedure TdfUserControl.NextTarget;
  ...
  function QuickDistanceTo(aObj: TdfGameObject): Single;
  var
    v1, v2: TAffineVector;
  begin
    v1 := FSpaceFighter.Position.AsAffineVector;
    v2 := aObj.Position.AsAffineVector;
    Result := sqr(v2[0] - v1[0]) + sqr(v2[1] - v1[1]) + sqr(v2[2] - v1[2]);
  end;

var
  i: Integer;
  obj, next: TdfGameObject;
  minDistance, qDistance: Single;
begin
  inherited;
...
//    FTarget.Target := TdfGameObject(FSpaceFighter.ObjectsAround[FCur]);
  {Ќемного переделаем код}
  next := nil;
  minDistance := INFINITE;
  for i := 0 to FSpaceFighter.ObjectsAround.Count - 1 do
  begin
    obj := TdfGameObject(FSpaceFighter.ObjectsAround[i]);
    if Assigned(obj) then
    begin
      qDistance := QuickDistanceTo(obj);
      if  qDistance < minDistance then
      begin
  ...
  begin
    ...
    for i := 0 to FhudDirections.Count - 1 do
      ...
      begin
        Visible := i < tmpInt;
        Rotation := (180 / pi) *
          VectorGeometry.Arccos(VectorAngleCosine(YVector, AffineVectorMake(dmx, dmy, 0)));
        if (dmx > 0) then
          Rotation := - Rotation;
...
begin
  ...
  if Assigned(FTarget.Target) then
    weaponfocus := FTarget.DistanceValue
  else
    weaponfocus := C_WEAPON_FOCUS_RANGE;
...
//    RotateSpaceFighter();
    ...
    begin
      Ft := Ft + deltaTime * C_RETURN_ROTATE_SPEED;
      if (Ft >= C_RETURN_ROTATE_TIME) or
         (Abs(VectorDotProduct(YVector, FSpaceFighter.Up.AsAffineVector) - 1) < C_RETURN_ROTATE_MINDISTANCE) then
      begin
      ...
      begin
        FSpaceFighter.Up.AsAffineVector := VectorLerp(FUpVec, YVector,
